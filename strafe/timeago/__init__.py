...
from timeago.excepts import ParameterInvalid
...
def format(date, now=None, locale='en'):
    ...
    if not isinstance(date, timedelta):
        ...
        if date is None:
            raise ParameterInvalid('the parameter `date` should be datetime '
                                   '/ timedelta, or datetime formated string.')
        if now is None:
            raise ParameterInvalid('the parameter `now` should be datetime, '
